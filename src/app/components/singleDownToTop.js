        ...
        <div className={styles.bracketsArea} style={{flexDirection:'column'}}>
                    ...
                    })}
                    {
                        tourneyData.joins.map((join,index)=>{
                            return (
                                <>
                                    <Xarrow key={index+'x1'} start={join[0]} end={join[2]} label={join[2]} color={tourneyData.settings.mainColor} strokeWidth={2} path={tourneyData.settings.strokeStyle} startAnchor={'top'} endAnchor={'bottom'}/>
                                    <Xarrow key={index+'x2'} start={join[1]} end={join[2]} label={join[2]} color={tourneyData.settings.mainColor} strokeWidth={2} path={tourneyData.settings.strokeStyle} startAnchor={'top'} endAnchor={'bottom'}/>
                                </>
                            )
                        })
                    }
